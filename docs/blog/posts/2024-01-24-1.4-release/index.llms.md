Quarto 1.4 has been officially released! You can get the current release from the [download page](../../../../docs/download/index.llms.md)
...
[![Screenshot of a Stock Trader dashboard: a row of three values boxes, then a row with a stock ticker graph and a table of closing values. Navy blue and green theme.](../../../../docs/dashboards/examples/thumbnails/stock-explorer-dashboard.png)](https://jjallaire.github.io/stock-explorer-dashboard/)

[![Screenshot of a Customer Churn dashboard: a row of three values boxes, then a row with two plots, then a row with a table. Light blue and yellow theme.](../../../../docs/dashboards/examples/thumbnails/customer-churn-dashboard.png)](https://jjallaire.github.io/customer-churn-dashboard/)

[![Screenshot of a Palmer Penguins dashboard: a sidebar with checkboxes and a dropdown, and two plots in main panel. Blue theme.](../../../../docs/dashboards/examples/thumbnails/penguins-dashboard.png)](https://jjallaire.shinyapps.io/penguins-dashboard/)

For the source code of these dashboards and additional examples see the [examples gallery](../../../../docs/gallery/index.llms.md#dashboards). When you are ready to build your own Quarto dashboard head to our guide on [Dashboards](../../../../docs/dashboards/index.llms.md).
...
We are particularly excited about how easy it is to make templates for journal articles, conference posters, newsletters and more with Typst. Here are some examples you can use in Quarto as [custom formats](../../../../docs/output-formats/typst-custom.llms.md):

[![Screenshot of a page showing a article styled according IEEE standards. The title is centered with authors below in two columns.](images/typst-format-ieee.png)](images/typst-format-ieee.png "IEEE")

IEEE

[![Screenshot of a poster in landscape orientiation. The poster includes a logo in the top right, a title in the top left, and content arranged in three columns.](images/typst-format-poster.png)](images/typst-format-poster.png "Poster")

Poster

[![Screenshot of a page showing a letter. A sender address is across the top of the page, followed by a recipient address left justified. The body of the letter includes a subject line in bold.](images/typst-format-letter.png)](images/typst-format-letter.png "Letter")

Letter

[![Screenshot of a page showing a department newsletter. The page is split vertically with a white column on the left and a red one on the right. An image spans across the column with the text "Award Winning Science" oriented to run vertically down its right side.](images/typst-format-dept-news.png)](images/typst-format-dept-news.png "Dept News")

Dept News

Start your Typst journey with Quarto in our guide on [Typst Basics](../../../../docs/output-formats/typst.llms.md).
...
And don’t worry if you are used to using `` `r ` `` that syntax remains fully supported. Read more at [Inline Code](../../../../docs/computations/inline-code.llms.md).

## Cross-References

Cross-references have been overhauled in Quarto 1.4, enabling you to do things like:

- Flexibly define the content of float cross-references (e.g. figures, tables and code listings) with the new [Cross-Reference Div Syntax](../../../../docs/authoring/cross-references-divs.llms.md). For example, [Table 1](#tbl-table) is an image treated like a table:
...
- Define [custom types of float cross-reference](../../../../docs/authoring/cross-references-custom.llms.md), which you could use to create cross-references to Videos, Diagrams or [Supplemental Figures](../../../../docs/authoring/cross-references-custom.llms.md#example-supplemental-figures).

- Cross-reference [executable code cells](../../../../docs/authoring/cross-references.llms.md#code-listings), [callouts](../../../../docs/authoring/cross-references.llms.md#callouts) and [remarks and solutions](../../../../docs/authoring/cross-references.llms.md#theorems-and-proofs).
...
Read more about manuscripts and how to get started in our guide to [Manuscripts](../../../../docs/manuscripts/index.llms.md).

## Other Highlights

Some other highlights include:

- [Shiny for Python](../../../../docs/dashboards/interactivity/shiny-python/index.llms.md)—Support for using Shiny for Python within Quarto documents.

- [Script Rendering](../../../../docs/computations/render-scripts.llms.md)—Render specially formatted `.py`, `.jl` and `.r` script files.

- [Easy Binder Configuration for Quarto Projects](../../../../docs/projects/binder.llms.md)—Support for generating files required to deploy a Quarto project to [Binder](https://mybinder.org/).

- [Connect Email Generation](https://docs.posit.co/connect/user/quarto/#email-customization)—Extends the `html` output format so that HTML/text emails can be created and selectively delivered through Posit Connect.

- [Publish to Posit Cloud](../../../../docs/publishing/posit-cloud.llms.md)—Adds `posit-cloud` as a venue for `quarto publish`.

- [Lightbox Treatment for Images and Figures](../../../../docs/output-formats/html-lightbox-figures.llms.md)—Support for zooming into images and figures as well as grouping multiple images into a gallery.

If you build Quarto extensions, you should also be aware of some developer-facing changes:

- [Lua changes](../../../../docs/prerelease/1.4/lua_changes.llms.md)—New Support for crossreferenceable elements in filters, extensible renderers of quarto AST nodes such as `FloatRefTarget` and `Callout`, the use of relative paths in `require()` calls, and more precise specification of where a filter will be inserted.

- [AST processing changes](../../../../docs/prerelease/1.4/ast.llms.md)—Improvements to the HTML table processing added in v1.3 and a way for LaTeX raw blocks to include Quarto-compatible markdown for processing.
...
The party popper emoji in the [listing and social card image](images/thumbnail.png) for this post comes from [OpenMoji](https://openmoji.org/)– the open-source emoji and icon project. License: [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/#)
