...
Script rendering for Jupyter makes use of the [percent format](https://jupytext.readthedocs.io/en/latest/formats-scripts.llms.md) that is supported by several other tools including Spyder, VS Code, PyCharm, and Jupytext.
...
Script rendering for Knitr is based on the `knitr::spin()` feature and makes use of the same [syntax rules](https://bookdown.org/yihui/rmarkdown-cookbook/spin.llms.md):
...
Notebook scripts can also be included within [projects](../../docs/projects/quarto-projects.llms.md) (e.g. websites, blogs, etc.). Scripts within projects are only rendered by Quarto when they start with the appropriate [syntax](#syntax).
