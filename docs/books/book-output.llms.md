...
HTML books are at their core [Quarto Websites](../../docs/websites/index.llms.md) with some special navigational behavior built in. This means that all of the features described for enhancing websites are also available for books, including:

- [Navbars](../../docs/websites/website-navigation.llms.md#top-navigation)
- [Social Metadata](../../docs/websites/website-tools.llms.md#social-metadata)
- [Full Text Search](../../docs/websites/website-search.llms.md)
- [Google Analytics](../../docs/websites/website-tools.llms.md#google-analytics)
- [Headers and Footers](../../docs/websites/website-tools.llms.md#headers-footers)
- [Dark Mode](../../docs/websites/website-tools.llms.md#dark-mode)
...
- Immediately below the table of contents on the right there is an “Edit this page” link that takes the reader to the edit interface on GitHub for the current chapter. Note that in this example we specify `repo-actions: [edit]`. You can optionally also add `issue` and `source` actions (e.g. `repo-actions: [edit, issue, source]`). There are additional options available (`repo-subdir` and `repo-branch`) for [customizing repository links](../../docs/websites/website-navigation.llms.md#github-links).

- The [Hypothesis](https://web.hypothes.is/) commenting bar appears on the far right of the page. Note that [commenting](../../docs/output-formats/html-basics.llms.md#commenting) is a feature available for all Quarto HTML output so appears in its own YAML key.

#### Sidebar Options

Note that books utilize the standard sidebar component from [Quarto Websites](../../docs/websites/index.llms.md). This means that you can use any of the available [sidebar options](../../docs/websites/website-navigation.llms.md#side-navigation) within your `book` configuration. For example, here we specify a docked sidebar with a light background:
...
You can learn more about creating theme files in the documentation on [HTML Themes](../../docs/output-formats/html-themes.llms.md).
...
Note that we also specify a cover image. To learn more about other EPUB options, see the Pandoc [documentation on EPUBs](https://pandoc.org/MANUAL.llms.md#epubs).

## PDF Styles

You can include additional LaTeX directives in the preamble of your book using the `include-in-header` option. You can also add `documentclass` and other options (see the Pandoc documentation on [LaTeX options](https://pandoc.org/MANUAL.llms.md#variables-for-latex) for additional details). For example:
...
You can customize MS Word output by creating a new [reference doc](https://pandoc.org/MANUAL.llms.md#option--reference-doc), and then applying it to your book as follows:
...
Learn more about creating and customizing a reference document in the documentation on [Word templates](../../docs/output-formats/ms-word-templates.llms.md).
