...
The Groff (GNU troff) man page document formats consists of plain text mixed with formatting commands that produce ASCII/UTF8 for display at the terminal. Learn more about Groff at [https://www.gnu.org/software/groff/groff.html](https://www.gnu.org/software/groff/groff.llms.md).
...
|  |  |
|:---|:---|
| `adjusting` | Adjusts text to the left, right, center, or both margins (`l`, `r`, `c`, or `b`). |
| `hyphenate` | Whether to hyphenate text at line breaks even in words that do not contain hyphens if it is necessary to do so to lay out words on a line without excessive spacing |
