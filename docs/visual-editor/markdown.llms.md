...
You can set these options within the **R Markdown** [Global Options](../../docs/visual-editor/options.llms.md#global-options) or [Project Options](../../docs/visual-editor/options.llms.md#project-options), or can alternatively set them on a per-file basis using YAML (as described below).
...
However, if you prefer to insert line breaks at a particular column (e.g. 72 or 80), or to insert a line break after each sentence, you can set a global or per-project [editor option](../../docs/visual-editor/options.llms.md#global-options) to this effect.
...
Note that you can also set a global or per-project [editor option](../../docs/visual-editor/options.llms.md#global-options) to control reference writing behavior.
...
> **NOTE:**
>
> Note that if you are within a Quarto [book](../../docs/books/index.llms.md) project then a references `prefix` is applied automatically so no changes to `editor: markdown` are required.
