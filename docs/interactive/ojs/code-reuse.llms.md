...
You may be using Python or R to pre-process data that is then provided to OJS via the `ojs_define()` function (this is described in more depth in the [Data Sources](../../../docs/interactive/ojs/data-sources.llms.md#python-and-r) article). If you want to share data prepared in this fashion you can import it directly from another `.qmd`.

For example, here we import the `co2data` that we read and pre-processed with dplyr in [`data-sources.qmd`](../../../docs/interactive/ojs/data-sources.llms.md):
...
You can import any reactive value from another `.qmd` file. Here, we’re reusing a chart directly from [`data-sources.qmd`](../../../docs/interactive/ojs/data-sources.llms.md):
