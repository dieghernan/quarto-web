...
But what if you want to do data transformation dynamically in response to user inputs? This is also possible with `ojs_define()`, as it can be passed not just static values but also Shiny reactives (assuming it’s running inside a Shiny [interactive document](../../../docs/interactive/shiny/index.llms.md)).

## Hello, Shiny

Here is the [K-Means Clustering](https://shiny.rstudio.com/gallery/kmeans-example.llms.md) example from the Shiny Gallery implemented with an OJS client and Shiny Server:

[![](images/kmeans-shiny-ojs.png)](https://jjallaire.shinyapps.io/kmeans-shiny-ojs/)
