...
![](html-code_files/figure-html/unnamed-chunk-1-1.png)
...
![A screenshot of this webpage with a pop-up window labeled 'Source Code' over it. This 'Source Code' window contains the raw markdown and R code used to write this page. There is an 'X' on the upper right corner of the 'Source Code' pop up to close it.](images/code-tools-source.png)
...
![A block of code with a gray background.](images/code-bg.png)
...
![A block of code with a gray vertical stripe running along its left border. This code block has no background.](images/code-default.png)
...
![A block of code with a gray background and a blue vertical stripe running along its left border.](images/code-custom.png)
...
The documentation on [computations](../../docs/get-started/computations/index.llms.md) covers how to include executable code blocks (code which is actually executed, with its output being included in the rendered document). We won’t additionally cover that here, but we will talk about how to include code blocks that demonstrate executable syntax (e.g. for writing a tutorial).
...
Code linking is currently implemented only for the knitr engine (via the [downlit](https://downlit.r-lib.org) package). A limitation of downlit currently prevents code linking if `code-line-numbers` and/or `code-annotations` are also `true`.
