...
This example highlights a few of the options available for HTML output. This document covers these and other options in detail. See the HTML [format reference](../../docs/reference/formats/html.llms.md) for a complete list of all available options.
...
![Screenshot of a HTML page shdwing the headers 'Other Links' and 'Code Links' below the Table of Contents on the right of the page. The 'Other Links' header is followed by a link with the text 'NASA Open Data', and the header 'Code Links' is followed by a link with the text 'Data Import Code'.](images/html-other-links.png)
...
`binder`  
Add a link to “Launch Binder” under “Code Links” if your project is configured to [Use Binder](../../docs/projects/binder.llms.md).
...
Using the `css` option works well for simple tweaks to document appearance. If you want to do more extensive customization see the documentation on [HTML Themes](../../docs/output-formats/html-themes.llms.md).
...
Note that there is more detailed documentation on each of these options in the Pandoc [Math Rendering in HTML](https://pandoc.org/MANUAL.llms.md#math-rendering-in-html) documentation.
...
By default the first tab in the tabset will be active when a page loads. To have a different tab active on page load, add the `.active` class to the tab heading. For example, here the “Python” tab will be active on page load:
...
Anchor links are also automatically added to figures and tables that have a [cross reference](../../docs/authoring/cross-references.llms.md) defined.
...
   Hover over Xie ([2015](#ref-xie2015)) to see a reference to the definitive book on knitr[^1]. [Listing 1](#lst-hover) shows you how to disable hover behaviour.
...
You can see the Hypothesis UI at the far right of the page. Rather than `true`, you can specify any of the available Hypothesis [embedding options](../../docs/reference/projects/websites.llms.md#hypothesis) as a sub-key of `hypothesis`. For example:
...
You can enable [Utterances](https://utteranc.es/) commenting using the `utterances` option. Here you need to specify at least the GitHub repo you want to use for storing comments:
...
You can also specify the other options [documented here](../../docs/reference/projects/websites.llms.md#utterances).
...
Review the [Giscus documentation](https://giscus.app) for instructions on setting up Giscus in your repository. Additional options are [covered here](../../docs/reference/projects/websites.llms.md#giscus).
