...
![Screenshot of Microsoft Word document open with Styles pane open in a pane over the left side of the document.](images/word-styles.png)
...
![Modify Style dialog box with a properties section for selecting what is to be styled, and a formatting section for selecting the style settings. The color selection dropdown in the formatting section is open.](images/word-modify-styles.png)
...
Overall, this mechanism allows for great customization by allowing authors to define new styles in the reference document. For advanced users, this could be coupled with [filters](../../docs/extensions/filters.llms.md) to apply a `custom-style` attribute to a filtered set of elements in the document.
