...
Note that Quarto’s automatic installation of missing TeX packages will work for TinyTeX and TeX Live, but not for other TeX distributions (as it relies on TeX Live’s [tlmgr](https://www.tug.org/texlive/tlmgr.llms.md) command).
...
| `latex-tlmgr-opts` | Array (options for [tlmgr](https://www.tug.org/texlive/tlmgr.llms.md)) |
