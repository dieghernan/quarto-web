...
The above example highlights a few of the options available for Typst output. This document covers these and other options in detail. See the Typst [format reference](../../docs/reference/formats/typst.llms.md) for a complete list of all available options.
...
Learn more about how to use them, and how to create your own in [Custom Formats](../../docs/output-formats/typst-custom.llms.md).

## Known Limitations

Since Typst is under active development, there are still some limitations to Quarto’s Typst support:

- The default size of images may not reflect the behavior you are used to in other output formats. This is a problem that Typst, pandoc and Quarto are actively working to fix. In the meantime, you can manually [specify image widths](../../docs/authoring/figures.llms.md#figure-sizing).

- Advanced page layout (e.g. using the `.column-*` classes as explained in [Article Layout](../../docs/authoring/article-layout.llms.md)) is not implemented.
...
![Screenshot of one page of PDF document. The document shows a two columns of text. Compared to the previous screenshot, the page is narrower and longer, and the margins are smaller.](images/typst-custom-page.png)

Customized Layout

![Screenshot of one page of PDF document. The document shows a single column of text.](images/typst-default-page.png)
...
The `page-numbering` option expects a string that describes a [Typst numbering](https://typst.app/docs/reference/model/numbering/) pattern. For example, the default, `1`, numbers pages with arabic numerals, and `i` numbers pages with lower case roman numerals. To omit page numbers, set `page-numbering` to `false`:
...
See [the Advanced documentation](../../docs/advanced/typst/typst-css.llms.md#supported-elements-and-properties) for supported elements and properties, and how to add more using Lua filters.
...
![](images/typst-tables-pandas-temps.png)
...
If there is a logo [from brand.yml](../../docs/authoring/brand.llms.md#logo) or specified directly in the document, Quarto’s Typst implementation allows customization of the logo position in the document (or project):
...
| `location` | Location on the page in `X-Y`, where `X` is `left`, `center`, or `right` and Y is `top`, `middle`, or `bottom`. Applied as Typst [`align`](https://typst.app/docs/reference/layout/align/). |
| `padding`, `padding-top`, `padding-right`, `padding-bottom`, `padding-left` | Amount of padding to add to each side of the logo, using CSS [`padding`](https://developer.mozilla.org/en-US/docs/Web/CSS/padding) conventions. Padding options are applied in alphabetical order. Applied as Typst [`block.inset`](https://typst.app/docs/reference/layout/block/#parameters-inset). Default `padding: 0.75in`. |
...
See other figure options on the [Typst reference page](../../docs/reference/formats/typst.llms.md#figures)
