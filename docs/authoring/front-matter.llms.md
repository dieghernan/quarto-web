...
![Screenshot of the JATS preview from the document with the above metadata.](images/scholarly-front-matter-jats.png)

## HTML

![Screenshot of the HTML preview from the document with the above metadata.](images/scholarly-front-matter-html.png)

Not all of the metadata keys are used in every format. However, the tags described on this page will generally be supported in [journal article formats](../../docs/extensions/listing-journals.llms.md). Currently the JATS format makes use of the broadest set of metadata tags, so if you want to check how things render we recommend previewing with `format: jats`.
...
Available keys to `author` {.caption-top .table}
...
>
> CRediT contributor values available in `roles` {.caption-top .table}
...
Available keys to `affiliation` {.caption-top .table}
...
The `citation` key allows you to specify additional metadata that is used to create a citation for the document. You can read more about this in [Creating Citeable Articles](../../docs/authoring/create-citeable-articles.llms.md).
