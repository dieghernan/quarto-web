...
![A navigation bar. The title 'My Site' is on the left. To the right of the title are the words 'Home', 'Talks', and 'About'. 'Home' is slightly lighter than the other two words. On the far right side of the navigation bar is a search box.](images/nav-bar.png)
...
For more information on controlling the appearance of the navigation bar using HTML themes, see [HTML Themes - Navigation](../../docs/output-formats/html-themes.llms.md#navigation).
...
![The right section of a Quarto navbar containing a Twitter and Github logo. The Github logo is selected and a menu is underneath it with two items: 'Source Code' and 'Report a Bug](images/navbar-tools.png)
...
|  |  |
|:--:|:--:|
| ![A screenshot of a Quarto document where the sidebar is colored gray.](images/nav-side-anchored.png) | ![](images/nav-side-floating.png) |
...
For more information on controlling the appearance of the side navigation using HTML themes, see [HTML Themes - Navigation](../../docs/output-formats/html-themes.llms.md#navigation). If you need to control the width of the sidebar, see [Page Layout - Grid Customization](../../docs/output-formats/page-layout.llms.md#grid-customization).
...
![A navigation bar titled 'ProjectX' on the left. To the right of the title are the menu items 'Home', 'Tutorials', 'How-To', 'Fundamentals', and 'Reference'. 'Home' is in a lighter color than the other menu options. The other menu options have a triangle pointing down next to each one, indicating the existence of a drop-down menu. There is a search bar on the far right side of the navigation bar.](images/nav-bar-hybrid-dropdown.png)
...
![Screenshot of a webpage. Above the header 'Tutorials' is the linked text 'Tutorials \> Tutorial Landing'.](images/nav-breadcrumbs.png)
...
Or to control page navigation for all pages in a directory specify `page-navigation` in [`_metadata.yml`](../../docs/projects/quarto-projects.llms.md#directory-metadata):
...
![Reader mode toggle appearing the top navigation.](images/reader-mode.png)
...
![A screen shot of a Quarto document. Underneath the page table of contents on the right side are three options: 'Edit this page', 'View source' and 'Report an issue'. There is a Github icon to the left of 'Edit this Page.'](images/repo-actions.png)
...
Note that if you are already providing a `site-url` (which is required for generation of sitemaps and [social metadata](../../docs/websites/website-tools.llms.md#social-metadata) preview images) then it’s enough to simply include the path within the `site-url`:
