...
Quarto passes the listing contents to the template as an array called `items`. Each item is populated with the fields described in [Listing Fields](../../docs/websites/website-listings.llms.md#listing-fields), plus any custom metadata from the document.
...
![A custom listing output showing a list of document titles as links.](images/listing-custom-output.png)
...
![](images/listing-custom-metadata.png)
...
Portions of this website are built using custom listings. The best place to start is with [our gallery](../../docs/gallery/index.llms.md), which is a listing built using a custom template and a metadata file. You can view the source code used to create the gallery page in our [Github repository](https://github.com/quarto-dev/quarto-web).
