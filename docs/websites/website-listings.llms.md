...
Review the [Quarto Glob Reference](../../docs/reference/globs.llms.md) for more information about supported glob syntax.
...
In addition to specifying lists of files or globs, contents can contain lists of metadata as well. For more about this, see [Custom Listings](../../docs/websites/website-listings-custom.llms.md).
...
![The default layout for listings page. The top is a header (Listing Example). Below on the left there is a sortable dropdown for the order in which the items are displayed. On the right, across, is a search input box. The items are listed full-width rows that display the item’s metadata (author and date), title, description, and image.](images/listing-default.png)

## Grid

Grid style listings display a card for each item.

![Listing Example page with grid-style layout cards for each item.](images/listing-grid.png)

## Table

The table listing style provides a traditional tabular layout.

![Listing Example with a text table displaying (from left to right): Date, Title, and Author.](images/listing-table.png)
...
![Default layout listings page with a sidebar on the right showing categories and the counts of items for each category.](images/listing-categories.png)
...
![Table with small image thumbnail for each item and custom list of fields: Date, Title, Author, and Reading Time.](images/list-fields-list.png)
...
![Default listings layout with the image in the right column, the title, subtitle, and description in the center column, and author, date, reading time, file, and file modified fields in the left column.](images/default-fields.png)
...
![Grid listing layout item with image thumbnail at the top of the card, followed below by the title, then subtitle, reading time, category tags, and description. Below the description there is a line with author and date, and another line with file and file modified.](images/grid-fields.png)
...
![Default layout listings page with a footer that reads 'Learn more about Quarto here'.](images/listing-example-id.png)
