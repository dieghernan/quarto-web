...
![A screenshot of a Quarto document with the title Using R - Doc in the RStudio Editor.](images/confluence-qmd.png)

A Quarto Document

![A screenshot of a document with the title Using R - Doc in a Confluence Space.](images/confluence-page.png)

Published to Confluence

![A screenshot of a Quarto project in VS Code. On the left in the Explorer, the project folder is called 'Guide-site', and contains folders 'authoring', and 'computation', along with some other files. A document from the folder 'python' inside the folder 'computations' with the title 'Using Python - site' is open in the Source Pane. ](images/confluence-project.png)

A Quarto Project

![A screenshot of Space in Confluence. On the left in the Sdiebar under Pages is a page called 'Guide-site'. Nested under this page are pages called 'authoring', and 'computation', along with some other pages. The 'computation' page item is expanded and shows a page called 'Using Python - site', nested under a page called 'python'. A page is displayed on the right with the title 'Using Python - site'](images/confluence-site.png)
...
![Screenshot of the result of previewing the file confluence-demo.qmd.](images/confluence-demo-preview.png)
...
![Screenshot of the published confluence-demo.qmd file on Confluence. In the sidebar, the page Confluence Demo is listed under Pages at the top level.](images/confluence-demo-published.png)
...
To publish a collection of documents, organize your documents in a [Quarto project](../../docs/projects/quarto-projects.llms.md), and use the `confluence` project type. Here’s a minimal `_quarto.yml` file for a Confluence project:
...
![A zoomed in view of the navigation sidebar in Confluence. Under Pages is a page called Project Example Site, nested under this page are pages called Team Members, Project Roadmap, and Reports-folder. Under the Reports-folder page are pages called: Reports, March, and January.](images/confluence-project-structure.png)
...
![Screenshot of a page in Confluence titled Reports, with a single sentence 'Monthly reports on project progress'. In the sidebar navigation this page is called 'Reports' and is nested under the page called 'Project Example Site'.](images/confluence-index.png)
...
![Screenshot of the Confluence permissions on a page, with two items: Everyone is set to View; and Charlotte Wickham (Me) is set to Can Edit](images/confluence-permissions.png)
...
Raw Confluence blocks allow you to include content that should pass through Quarto unchanged and be interpreted directly by Confluence. For example, [Confluence’s Storage Format](https://confluence.atlassian.com/doc/confluence-storage-format-790796544.llms.md) includes specific tags for a task list. To include a Confluence task list in your document, use these tags inside a raw Confluence block:
...
![A screenshot of list with one item. To the left of the item text is an unchecked checkbox.](images/confluence-task-list.png)
