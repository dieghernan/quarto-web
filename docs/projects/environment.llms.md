...
Quarto projects can define a set of standard environment variables as well as vary which environment variables are defined based on the [active project profile](../../docs/projects/profiles.llms.md). It’s also possible to use environment variables that are defined only on the local system and not checked in to version control.
...
If you are using [Project Profiles](../../docs/projects/profiles.llms.md) to adapt your projects for different scenarios, you can create a custom environment file that will be activated along with the profile. For example, if you have a profile named `production`, then the variables defined in `_environment-production` will be read and applied.
