...
The programming language used to create [filters](../../docs/extensions/filters.llms.md) and [shortcodes](../../docs/extensions/shortcodes.llms.md) is Lua, a lightweight, high-level scripting language. [Lua](https://www.lua.org/) is the extension language for Pandoc (which includes an embedded Lua interpreter). This means that Quarto extensions have no additional runtime dependencies or requirements.
...
See the [Lua API Reference](../../docs/extensions/lua-api.llms.md) for additional details on the APIs available for developing extensions.
...
2.  Check out the first two sections of the [Pandoc Lua Filters](https://pandoc.org/lua-filters.llms.md) documentation then skip ahead to the [Filter Examples](https://pandoc.org/lua-filters.llms.md#macro-substitution) section to make things a bit more concrete.

3.  Once you have the basic idea of Lua and filters, get a more complete picture by skimming the full [Pandoc Lua Filters](https://pandoc.org/lua-filters.llms.md) documentation. You won’t understand everything, but it’s a good orientation to all of the moving parts.
...
![Screenshot of Lua file open in VS Code with code completion popup showing for Lua function.](images/vscode-lua-completions.png)
...
![Screenshot of Lua file open in VS Code with diagnostic errors shown in Problems pane below the code editor.](images/vscode-lua-diagnostics.png)

#### Installation

To get started with using VS Code or Positron for Lua extension development, install the following software:

1.  Install the latest version (v1.2 or greater) of [Quarto](../../docs/download/index.llms.md)
