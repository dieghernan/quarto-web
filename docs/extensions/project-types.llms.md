...
If you additionally include some basic scaffolding as a [Starter Template](../../docs/extensions/starter-templates.llms.md) for using the project type, and host it within a GitHub repository, then users could get a new project up and running as simply as:

``` bash
quarto use template lexcorp/lexdocs
```

Note that it is possible to bundle and distribute project type extensions as simple gzip archives (as opposed to using a GitHub repository as described above). See the article on [Distributing Extensions](../../docs/extensions/distributing.llms.md) for additional details.
...
- `theme.scss` provides a [custom theme](../../docs/output-formats/html-themes.llms.md) for HTML output.
...
Quarto includes a couple of built-in project-types for integrating with the [Hugo](../../docs/output-formats/hugo.llms.md) and [Docusaurus](../../docs/output-formats/docusaurus.llms.md) publishing systems. You can see the source code for these project types here:
