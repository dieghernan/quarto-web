...
To follow this tutorial you’ll need your own copy of the [template repository](https://github.com/quarto-ext/manuscript-template-vscode), including all of its branches.

1.  Head to [GitHub to create a new repository from the template](https://github.com/quarto-ext/manuscript-template-vscode/generate).
...
![A screenshot of VS Code. The File Explorer is open showing the folders: \_extensions, \_freeze, .github, images, and notebooks; and the files: \_quarto.yml, index.ipynb, README.md and references.bib.](images/vscode-open.png)

Template manuscript open in VS Code
...
Preview the manuscript by hitting the **Preview** button (![Preview icon](../../../docs/tools/images/vscode-preview-icon.svg)) located in the menu bar of the editor:

![Screenshot of the source editor menu bar with the Preview button highlighted.](images/vscode-render-button.png)

You’ll see some output from Quarto in the Terminal and then a live preview will appear in a new Quarto Preview pane.

![Screenshot of the VS Code. Open in an Editor pane is a file called index.qmd. Open in a pane titled Quarto Preivew is a website starting with the header La Palma Earthquakes.](images/vscode-preview.png)
...
![Screenshot of VS Code. Open in an Editor pane is a file called index.qmd with the text, title: La Palma Earthquake Mechanisms. In the Quarto Preview pane is an article webpage with the title La Palma Earthquake Mechanisms.](images/vscode-edit.png)

## Visual Editor

The VS Code visual editor provides a [WYSIWYM](https://en.wikipedia.org/wiki/WYSIWYM) editing interface for Quarto notebooks. Some tasks, like adding citations, or creating tables, are easier in the visual editor - we’ll point these out as we introduce them.

![Screenshot of the file \`index.qmd\` open in the Source Editor. File shows line numbers and the raw markdown for inserting a figure and table.](images/source-editor-vscode.png)

Source Editor

![Screenshot of the file \`index.qmd\` open in the Visual Editor. File shows a figure and table.](images/visual-editor-vscode.png)

Visual Editor

To toggle between source and visual editor modes, use the Editor menu and select “Edit in Visual Mode” or “Edit in Source Mode” mode, or use the keyboard shortcut .

![Screenshot of the Editor menu in VS Code. The 'Edit in Visual Mode' item in the '...' menu is highlighted.](images/source-visual-menu-vscode.png)

There are other ways to switch modes, read about them, and other features of the visual editor at [Visual Editing in VS Code](../../../docs/tools/vscode/visual-editor.llms.md).
...
An alternative to including computations directly in the article notebook is to embed output from other notebooks. This manuscript project includes the notebook `data-screening.qmd` in the `notebooks/` folder.
...
The double curly braces (`{{`) and angle brackets (`<`) indicate this is a shortcode. The `embed` shortcode requires a path to a notebook chunk. In this case, it’s the file path to a source notebook, `data-screening.qmd`, followed by `#` and a chunk label. The chunk label is set using the Quarto chunk option `label` in `data-screening.qmd` notebook:
...
Whenever you change the content of `data-screening.qmd`, including updating chunk options like the caption, it will be re-rendered. Consequently, the code in `data-screening.qmd` will also be re-run.
...
``` bash
processing file: data-screening.qmd
                                                                                    output file: data-screening.qmd

Rendering output notebook [notebooks/data-screening.qmd]
Rendering HTML preview [notebooks/data-screening.qmd]
```
